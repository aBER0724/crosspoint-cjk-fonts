...
現在のカタログには、簡体字中国語、繁体字中国語、日本語向けのフォントが16ファミリーあります。Pages は選択された UI 言語に応じて元の名称またはローカライズ名を表示し、その下に安定した ASCII ビルド ID を残します。検索はどちらの名称にも対応します。カタログメタデータでは、ファミリーを `reader`（既定）または `ui` に分類できます。

- UI ファミリーは、8/10/12 pt の個別ダウンロードボタンではなく、ローカライズされた「UI フォント」ラベルを表示します。
- Reader ファミリーは設定済みの本文サイズを表示し、固定の 8/10/12 pt UI fallback ファイルをダウンロードボタン一覧から除外します。
- 標準カタログビルドでは、CJK UI fallback 用の 8/10/12 pt と、本文用の 14/16/18/22 pt を引き続き生成します。
...
非公開フォントは [CrossPoint CJK Font Maker](https://github.com/aBER0724/crosspoint-cjk-font-maker) で1つの `.cpfontpkg` ファミリーパッケージに変換できます。パッケージは物理サイズごとに1つの `.cpfont v4` を保持し、UI サイズ 8/10/12 pt を常に含み、本文サイズには任意の正の整数リスト（既定 14/16/18/22 pt）を指定できます。
