...
当前目录包含 16 个字体家族，覆盖简体中文、繁体中文和日文。Pages 会根据所选界面语言显示字体的原始名称或本地化名称，并在下方保留稳定 ASCII 构建 ID；搜索支持两种形式。目录元数据可将字体家族标记为 `reader`（默认）或 `ui`。

- UI 字体家族只显示本地化的“UI 字体”标识，不单独展示 8/10/12 pt 下载按钮。
- 阅读字体家族显示配置的阅读字号，同时从下载按钮列表中隐藏固定的 8/10/12 pt UI fallback 文件。
- 标准目录构建仍会生成 8/10/12 pt CJK UI fallback，以及 14/16/18/22 pt 阅读正文字体。
- 14/16/18/22 pt 映射到阅读器的四个持久化字号档位；默认竖屏边距下，每行约可显示 16/14/12/10 个全角 CJK 字符。
- 固件只选择已安装的物理字体文件，不会在设备端缩放 CJK 字体。

私人字体可通过 [CrossPoint CJK Font Maker](https://github.com/aBER0724/crosspoint-cjk-font-maker) 生成一个 `.cpfontpkg` 字体家族包。包内仍按每个物理字号保存一个 `.cpfont v4` 文件，固定包含 8/10/12 pt UI 字号，并允许输入任意正整数阅读字号列表（默认 14/16/18/22 pt）。
